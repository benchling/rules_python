...
func (py *Python) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	if cfg.PerFileGeneration() {
		if args.File != nil {
			for _, r := range args.File.Rules {
				if kindMatches(args.Config, r, pyLibraryKind) {
		...
		for libName, libSrcs := range existingMultiFileLibs {
			...
			if autoIncludeInit && !currentSrcsForLib.Contains(pyLibraryEntrypointFilename) {
		...
		if args.File != nil {
			for _, r := range args.File.Rules {
				...
				} else if kindMatches(args.Config, r, pyTestKind) {
					srcsList := r.AttrStrings("srcs")
					if len(srcsList) > 0 {
						// Skip the target that would be automatically generated by Gazelle
						// We only want to track custom targets
