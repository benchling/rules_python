...
func (py *Python) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	type preservedTargetSrcs struct {
		name string
		srcs *treeset.Set
	}

	isAutoGeneratedPerFileName := func(name string, srcs *treeset.Set) bool {
		var nonInitSrc string
		nonInitSrcCount := 0
		srcs.Each(func(_ int, src interface{}) {
			srcName := src.(string)
			if srcName != pyLibraryEntrypointFilename {
				nonInitSrc = srcName
				nonInitSrcCount++
			}
		})
		if nonInitSrcCount > 1 {
			return false
		}
		if nonInitSrc == "" {
			return name == strings.TrimSuffix(pyLibraryEntrypointFilename, ".py")
		}
		return name == strings.TrimSuffix(filepath.Base(nonInitSrc), ".py")
	}
	isAutoGeneratedLibraryName := func(name string, srcs *treeset.Set) bool {
		if cfg.PerFileGeneration() {
			return isAutoGeneratedPerFileName(name, srcs)
		}
		return name == cfg.RenderLibraryName(packageName)
	}
	isAutoGeneratedTestName := func(name string, srcs *treeset.Set) bool {
		if cfg.PerFileGeneration() {
			return isAutoGeneratedPerFileName(name, srcs)
		}
		return name == cfg.RenderTestName(packageName)
	}

	preservedSrcsForKind := func(kind string, availableSrcs *treeset.Set, shouldSkip func(string, *treeset.Set) bool) []preservedTargetSrcs {
		var preserved []preservedTargetSrcs
		if args.File == nil {
			return preserved
		}
		for _, r := range args.File.Rules {
			if !kindMatches(args.Config, r, kind) {
				continue
			}
			srcs := treeset.NewWith(godsutils.StringComparator)
			for _, src := range r.AttrStrings("srcs") {
				if filepath.Ext(src) != ".py" || !availableSrcs.Contains(src) {
					continue
				}
				srcs.Add(src)
			}
			...
			if shouldSkip(r.Name(), srcs) {
				continue
			}
			srcs.Each(func(_ int, src interface{}) {
				availableSrcs.Remove(src)
			})
			preserved = append(preserved, preservedTargetSrcs{name: r.Name(), srcs: srcs})
		}
		return preserved
	}
	...
	for _, target := range preservedSrcsForKind(pyLibraryKind, pyLibraryFilenames, isAutoGeneratedLibraryName) {
		appendPyLibrary(target.srcs, target.name)
	}
	...
	for _, target := range preservedSrcsForKind(pyTestKind, pyTestFilenames, isAutoGeneratedTestName) {
		pyTestTargets = append(pyTestTargets, newPyTestTargetBuilder(target.srcs, target.name))
	}
